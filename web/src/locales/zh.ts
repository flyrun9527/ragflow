...
<li><b>正则表达式分块：</b>支持使用自定义正则表达式进行精确内容分割</li>
</ul>
<p>此方法专门用于处理 MinerU 提取的 Markdown 文件，对这些文件进行基于标题层级的切块，保持文档的结构和上下文。</p>
`,
      regexPattern: '正则表达式',
      regexPatternTip:
        '自定义用于内容分割的正则表达式。当指定后，系统将在匹配该模式的位置分割内容。',
      regexPatternPlaceholder: '例如：^第[0-9]+条',
      regexPatternHint: '该模式将应用于每行的开头。使用 ^ 匹配行的开始。',
      regexPatternMessage: '正则表达式是必填项',
      strictRegex: `<p>支持的文件格式包括 <b>PDF</b>、<b>DOCX</b>、<b>TXT</b>、<b>MD</b> 和其他文本格式。</p>
<p>此方法使用自定义正则表达式在匹配该模式的位置分割内容。它特别适用于具有一致性章节标题的结构化文档，如法律法规、规章制度或技术手册。</p>
<p>特性：</p>
<ul>
<li><b>基于模式的分块：</b>使用自定义正则表达式作为主要分块边界</li>
<li><b>逐行处理：</b>将模式应用于每行的开头</li>
<li><b>动态大小控制：</b>遵循最大和最小token参数进行块大小控制</li>
<li><b>回退机制：</b>如果正则表达式失败或没有匹配项，则回退到智能分块</li>
</ul>
<p>此方法非常适合具有明确节段标记的文档，这些标记可以通过正则表达式匹配。</p>`,
