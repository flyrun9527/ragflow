...
from rag.app.utils import split_markdown_to_chunks_strict_regex, split_markdown_to_chunks_smart
import tiktoken
        ...
        # 处理PDF文件
            ...
            if layout_recognize == "Plain Text":
                ...
                if binary is not None:
                    ...
                    try:
                        sections, tbls = pdf_parser(temp_path, from_page=kwargs.get('from_page', 0), 
                                                to_page=kwargs.get('to_page', 1000), 
                                                callback=kwargs.get('callback', None))
                    finally:
                        import os
                        try:
                    ...
                    sections, tbls = pdf_parser(filename, from_page=kwargs.get('from_page', 0), 
                                            to_page=kwargs.get('to_page', 1000), 
                                            callback=kwargs.get('callback', None))
                ...
                if binary is not None:
                    ...
                    with tempfile.NamedTemporaryFile(suffix='.pdf', delete=False) as temp_file:
                        ...
                        temp_path = temp_file.name
                    try:
                        sections = pdf_parser(temp_path, from_page=kwargs.get('from_page', 0), 
                                          to_page=kwargs.get('to_page', 1000))
                    finally:
                        import os
                        try:
                    ...
                    sections = pdf_parser(filename, from_page=kwargs.get('from_page', 0), 
                                      to_page=kwargs.get('to_page', 1000))
                content = "\n\n".join(sections)
