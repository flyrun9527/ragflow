...
def get_blocks_from_md(md_file_path):
    ...
    try:
        ...
        _blocks_cache[md_file_path] = []
        return []
def get_blocks_from_md_middle(middle_json):
    block_list = []
    data = json.loads(middle_json)
    logging.info(f"data: {data}")
    logging.info(f"data-pdf_info: {data['pdf_info']}")
    # 检查数据结构类型
    if 'pdf_info' not in data:
        print(f"[WARNING] 无效的数据结构: 缺少 pdf_info 字段")
        return []
    ...
    print(f"[INFO] 提取了{len(block_list)}个块")
    return block_list
...
def get_bbox_for_chunk_middle(middle_json, chunk_content, block_list=None, matched_global_indices=None):
    """
    根据 md 文件路径和 chunk 内容，返回构成该 chunk 的连续 block 的 bbox 列表。
    采用 difflib.SequenceMatcher 找出最相似的 block（相似度最高），
    然后从该锚点向前后扩展，寻找同样存在于 chunk 中的连续 block。
